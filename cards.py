...
class Cards(object):
    ...
    def new(self, name, idList, desc=None,pos=None):
        resp = requests.post("https://trello.com/1/cards" % (), params=dict(key=self._apikey, token=self._token, pos=pos), data=dict(name=name, idList=idList, desc=desc))
        resp.raise_for_status()
        return json.loads(resp.content)
